...
public class SimulationService {
	public DataTablesModel getPageExercise(int pageNum, int pageSize, QueryExercise queryExercise) {
		...
		DataTablesModel exercisepage = Exercise.dao.pageDataTables(pageNum, pageSize, queryExercise,1);
			...
			if (rows.size() > 0) {
				for (int i = 0; i < rows.size(); i++) {
					...
					if(Integer.parseInt(row.get(4))>Integer.parseInt(row.get(5))){
						startstr = "| <a href='#' onclick=startExercise('" + row.get(0).trim() + "') >开始模拟考试</a>";
					}
