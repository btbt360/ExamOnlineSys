...
public class ExampapersService {
		...
		if (invigilatepage != null && !invigilatepage.equals("")) {
			...
			if (rows.size() > 0) {
				for (int i = 0; i < rows.size(); i++) {
					...
					row.set(5, Integer.parseInt((row.get(6) + "")) == 1 ? "<font color='#00ff66'>启用</font>"
							: "<font color='#C9C9C9'>禁用</font>");
					eqlist = ExampapersQuestion.dao
							.find("select * from sys_exampapers_question where exampapers_id = '" + id + "'");
					int endfinish = 0;
					...
					if (examlist.size() > 0) {
						...
						row.set(6, ischouqu + "<a href ='#' onclick=edit('" + id
	...
	public int getIsScoreAndIsTotal(String exampapersid, String questiontypeid, Double score) {
		...
		if (!TypeChecker.isEmpty(exampapersid) && !TypeChecker.isEmpty(questiontypeid)) {
			listtype = ExampapersQtypes.dao.find(
					"select * from sys_exampapers_qtypes where exampapers_id = ? and type_id =? ", exampapersid,
					questiontypeid);
			listequestion = ExampapersQuestion.dao.find(
					"select t.* from sys_exampapers_question t,sys_questions t1 where t.exampapers_id = ? and t1.id= t.question_id and t1.questiontype = ? ",
					exampapersid, questiontypeid);
			if (listtype.size() > 0) {
				eqt = listtype.get(0);
			}
	...
	public int goAutochoose(String exampapersid, String subjectid, String questiontypeid) {
		...
		listquestion = Questions.dao.find(
				"select * from sys_questions where subject_id = ? and questiontype = ? and isdel = 0 and isenable = 1 ",
		...
		listeq = ExampapersQuestion.dao.find(
				" select t.* from sys_exampapers_question t,sys_questions t1 where t.exampapers_id = ? "
				+ " and t1.isdel = 0 and t1.isenable = 1  and t.question_id = t1.id and t1.questiontype = ? ",
				exampapersid, questiontypeid);

		// 2.根据试卷id和试题类型id到试卷试题类型对照表中查询总题数和总分数
		List<ExampapersQtypes> listeqt = ExampapersQtypes.dao.find(
				" select * from sys_exampapers_qtypes where exampapers_id = ? and type_id =? ", exampapersid,
				questiontypeid);
		ExampapersQtypes eqt = new ExampapersQtypes();
		if (listeqt.size() > 0) {
	...
	public void saveExampapersQuestions(List<Questions> list, String exampapersid, Double score) {
		if (list.size() > 0) {
			for (Questions q : list) {
				...
				List<ExampapersQuestion> listeq = new ArrayList<ExampapersQuestion>();
				listeq = ExampapersQuestion.dao.find(
						"select * from sys_exampapers_question where exampapers_id = ? and question_id = ? ",
						exampapersid, q.getId());
				if (listeq.size() > 0) {
					continue;
				} else {
	...
	/**
	 * List<Object[]> list = new ArrayList<Object[]>(); list = Db.query(
	 * "select t1.id,t2.questiontype from sys_exampapers_question t1,sys_questions t2 where t1.exampapers_id=? and t2.id = t1.question_id "
	 * ,exampapersid); if(list.size()>0){ for(Object[] occ:list){ int oint = 0;
	 * oint=Integer.parseInt(occ[1]+""); switch (oint) { case 1://单选题 分支一;
	 * break; case 2: //多选题 分支二; break; case 3: //判断题 分支三; break; case 4: //问答题
	 * 分支三; break; default ://填空题 ; } } }
	 * 
	 */
	public void toFinishChoose(String exampapersid) {
		...
		for (int i = 1; i < 6; i++) {
			...
			list = ExampapersQuestion.dao
					.find("select t1.* from sys_exampapers_question t1,sys_questions t2 where t1.exampapers_id = ? and t2.questiontype = ? "
							+ " and t2.id = t1.question_id and t2.isdel = 0 and t2.isenable = 1 ", exampapersid, i);
	...
	public int goResetAutochoose(String exampapersid, String subjectid, String questiontypeid) {
		...
		list = ExampapersQuestion.dao
				.find("select t1.* from sys_exampapers_question t1,sys_questions t2 where t1.exampapers_id = ? and t2.questiontype = ? "
						+ " and t2.id = t1.question_id and t2.isdel = 0 and t2.isenable = 1 ", exampapersid, questiontypeid);
		if (list.size() > 0) {
			for (ExampapersQuestion eq : list) {
				eq.delete();
			}
